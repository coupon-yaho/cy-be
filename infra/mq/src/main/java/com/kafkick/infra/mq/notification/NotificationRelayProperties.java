...
public class NotificationRelayProperties {
     ...
     *
     * <pre>
     *   워커  1   123 ~ 128건/s     요청 p99  0.46 ~ 0.61ms
     *   워커  2   242 ~ 249건/s     요청 p99  0.47 ~ 0.95ms
     *   워커  4   443 ~ 458건/s     요청 p99  0.69 ~ 0.80ms
     *   워커  8   768 ~ 812건/s     요청 p99  0.85 ~ 1.01ms   ← 기본값
     *   워커 16  1296 ~ 1404건/s    요청 p99  6.6 ~ 7.8ms
     *   워커 32  1759 ~ 1890건/s    요청 p99 13.7 ~ 15.7ms
     * </pre>
     *
     * <p><b>처리량은 32 까지도 안 평평해진다.</b> CY-906 이 <i>"진짜 Kafka 에서는 브로커
     * 왕복이 먼저 평평해진다"</i> 고 적어 뒀는데 <b>그 예상이 빗나갔다</b> — 먼저 무너진
     * 것은 브로커가 아니라 <b>같은 프로세스가 공유하는 자원</b>이다.
     *
     * <p><b>요청 지연은 8 까지 평평하다.</b> 워커 1 과 워커 8 의 p99 가 같은 수준이고
     * 16 에서 일곱 배가 된다. <b>8 은 이제 비용에서 고른 값이 아니라 잰 값이다.</b>
     *
     * <h2>무너지는 것은 <b>커넥션 풀</b>이다 — CPU 가 아니다</h2>
     *
     * <p>⚠️ 처음엔 코어 수(11)와 절벽(16)이 가까운 것만 보고 <b>"CPU 초과 예약" 이라고
     * 단정했다가 틀렸다</b>(리뷰가 짚었다). 재 보니 워커 32 에서도 프로세스 CPU 는
     * <b>평균 8~9%</b> 다 — 스레드 48 개가 계산이 아니라 <b>기다리고 있다.</b>
     *
     * <p><b>풀만 13 → 64 로 바꾸니 절벽이 사라졌다.</b> 워커 32 의 요청 p99 가
     * 13.7~15.7ms → <b>2.25ms</b>, 처리량은 되레 1,996건/s 로 늘었다. 코어 수는 그대로다.
     * 기전은 단순하다 — 워커는 발행 후 outbox 를 갱신하느라 <b>커넥션을 하나씩 잡고</b>,
     * 워커가 풀보다 많으면 릴레이만으로 풀이 차서 <b>접수 요청이 커넥션을 기다린다.</b>
     *
     * <p><b>그래서 규칙은 코어가 아니라 풀이다.</b> 배포와 같은 조건(풀 13,
     * <b>요청 스레드 15</b> — {@code TOMCAT_THREADS_MAX})에서 재니 워커 9 는 워커 8 과
     * 구분이 안 되고 <b>워커 10 부터 요청 p99 분산이 터진다</b>(1,595~5,451µs).
     * 규칙은 <b>{@code workerCount + 4 ≤ 풀 크기}</b> 이고
     * {@link RelayWorkerPoolHeadroomGuard} 가 기동 때 못 박는다(CY-923·931).
     * 기본값은 8 + 4 = 12 ≤ 13 으로 통과하지만 <b>여유가 1 뿐이다.</b>
     *
     ...
     *
     * <p><b>남은 한 번의 빌림({@code markPublished})은 안 묶는다.</b> 상한을 재 보니
     * 그 쓰기를 <b>통째로 없애도 +25%</b> 였는데(818~956 → 1,084~1,137건/s), 같은 문서가
     * 잰 <b>풀 13 → 64 · 워커 32 는 1,996건/s(+120%)를 설정 두 줄로</b> 낸다. 게다가 묶으면
     * {@code PUBLISHED} 표시가 늦어져 <b>중복 발송 창이 플러시 간격만큼 넓어진다.</b>
     *
