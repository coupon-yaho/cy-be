...
public final class RedisQueueGatewayStateAdapter implements QueueGatewayStatePort {
     ...
     *
     * <p><b>어느 경로로 그 타입이 됐는지는 이 코드가 모른다.</b> 스크립트는 키의 지금 타입만
     * 본다 — 그 키들을 다른 형식으로 쓰던 Redis 에 이 공급기를 붙였을 수도, 수동 운영 명령이나
     * 다른 writer 가 만들었을 수도 있다. 원인을 좁히지 말고 위 네 부류 키의 {@code TYPE} 을
     * 먼저 보는 데서 시작한다.
     *
