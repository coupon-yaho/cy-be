 ...
 *
 * <ul>
 *   <li>커서 경로가 커서 자신을 다시 돌려주지 않는지. <b>배타 경계를 드라이버에 맡기지 않는다</b> —
 *       {@code readFrom} 은 {@code Range.closed} 로 커서를 <b>포함해</b> 읽고(첫 항목이 커서
 *       자신인지가 만료 판정이다) {@code readAfter} 가 그 한 건을 잘라 낸다. 그 잘라 내기가
 *       빠지면 폴링마다 마지막 항목이 한 번씩 더 나가 화면에 중복이 쌓인다. mock 은 조회도
 *       잘라 내기도 아예 실행하지 않는다.</li>
