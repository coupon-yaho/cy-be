...
#
# **DB 에 쓰지 않는다.** SELECT 둘뿐이고, 결과를 파일로 떨군 뒤 판정은 reconcile.py 가
# 한다. 판정을 파이썬에 두는 이유는 픽스처 파일만으로 시험할 수 있어서다 —
# 양쪽 동시 누락도, 재기동도, 늦은 취소 경합도 DB 없이 태울 수 있다.
...
if [[ -f "$REP/db-idempotency.tsv" ]] \
  ...
  log "⚠️ 멱등 덤프가 상한 100000 에 닿았다. 미완료 행이 그만큼 쌓였다는 뜻이다 —
      대조 전에 그쪽부터 본다. 센티널을 지워 판정 불가로 만든다"
