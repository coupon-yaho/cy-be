...
**요청을 보내기 전에** 접수 키·**대상·내용**을 적는 것이 핵심이다. 요구사항 §6.3 이
그 셋을 따로 적는다 — 대상만 맞대면 **엉뚱한 내용으로 발급된 건이 일치로 읽힌다.**
cy-be 에서 그 "내용" 은 서버가 해시하는 셋째 값(등급)이고, 옮기면 그 자리에 모델·옵션이
들어간다.

> 내용을 안 담은 옛 기록도 **거절하지 않는다** — 옛 회차를 다시 대조하는 것이 이
> 도구의 쓰임새다. 대신 `CONTENT_MISMATCH` 를 판정하지 않고 `대조 판정 불가` 로 남긴다.
> 없는 값을 "맞다" 로 세는 것이 이 도구가 가장 하면 안 되는 일이다. 성공 응답만 모으면
...
| `CONTENT_MISMATCH` | **요청한 내용과 저장된 내용이 다르다.** 원인이 위와 다르다 — 위는 응답↔저장, 이쪽은 **요청↔저장** |
...
> ⚠️ **전후 차이는 상태가 변했다는 사실이지 변한 이유가 아니다.** 요구사항 §6.3 이
> 그것을 못박는다 — *"자동 복구를 주장하려면 해당 키의 재시도·처리 이력도 확인"*.
>
> 미해결이 지워진 이유는 늦게 들어온 등록일 수도, 다른 경로의 보상일 수도, 사람이
> 손댄 것일 수도 있다. **스냅샷 둘로는 못 가른다.** 이유를 보려면 그 키의 처리
> 이력(`issuance_histories` 의 상태 전이)을 따로 읽는다.
>
> 대조는 그 판정을 **하지 않는다.** 같은 절이 *"미정인 조건을 검사기가 임의로
> 확정하지 않는다"* 고 적고, 원인 판정은 그 선을 넘는다.
