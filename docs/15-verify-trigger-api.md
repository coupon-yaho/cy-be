...
| **했다 (CY-947)** | `GET /api/v1/admin/verify/reports/residual?before=&after=` — 같은 둘을 **대상 단위**로 맞대 규칙별 `persisted`·`introduced`·`resolved` 를 낸다. **`diff` 로는 못 가르는 것이 있다**: `STOCK_MISMATCH 3 → 3` 이 *"그 3건이 그대로"* 인지 *"3건 고쳐지고 새로 3건"* 인지 응답이 같은 모양이고, 앞엣것은 아무도 안 고치고 있는 것이고 뒤엣것은 지금도 계속 깨지고 있는 것이라 **처방이 정반대다**. `remaining = persisted + introduced` 가 사전예약 PRD 대사 보고서의 **잔여 불일치 건수**다. 가드는 `diff` 와 같은 것을 그대로 지난다(같은 실행 400, 미종료 409, 시드 행 404). 집계는 **DB 가 접는다** — 규칙당 상한 10,000 × 규칙 여섯이라 두 실행이면 최대 12만 키인데 이 조회 예산은 5초다. **실측(CY-949): 상한 12만 키에서 약 270ms — 예산의 5%다.** `uk_run_finding` 을 **커버링**으로 타고(`using_index: true`) 임시 테이블이 안팎에 붙되 `filesort` 는 없다. ⚠️ **접근 방식은 비율을 따른다** — 대상이 표의 67%면 전체 커버링 스캔, 29%면 `range` 이고 **소요는 275ms / 268ms 로 같다.** 즉 비용은 표 크기가 아니라 **대상 행 수**를 따른다. `(finding_type, target_key)` 인덱스는 **안 만든다** — 마이그레이션이라 시드 DDL 과 함께 가야 하는데 지금 살 이유가 없다. 규칙당 상한을 크게 올리면 `ResidualQueryCostProbe` 를 다시 돌린다. 판정은 한 키가 한 실행에 두 번 못 나오는 성질로 `sides` 1/2 를 가르고, 키 비교는 **바이트 기준**이다 — 기본 콜레이션은 대소문자를 무시해 다른 검출을 한 키로 합친다. **그 유니크가 사라지면 지속을 과대 보고한다** |
