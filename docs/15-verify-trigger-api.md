...
| **했다 (CY-938)** | `GET /api/v1/admin/batch/runs/stuck` — **배포된 전 잡**의 시체 목록. 잡 이름으로 묶어서 주고, 시체가 없는 잡도 빈 그룹으로 낸다 — 빼면 "봤는데 없었다" 와 "안 봤다" 가 응답에서 같아진다. `POST .../runs/{executionId}/abandon` 이 CY-927 로 범용 관제에 올라왔는데 **그 `executionId` 를 찾을 조회가 잡별 경로(`/admin/expire`·`/admin/cleanup`)에만 있었다.** 잡 이름은 `Job` 빈에서 받는다 — `JobRepository.getJobNames()` 는 인터페이스 기본 구현이 `Collections.emptyList()` 라(6.0.4 바이트코드 실측) 상속이 끊기는 날 **"시체 0건" 을 영원히 참으로** 낸다 |
| **했다 (CY-944)** | `GET /api/v1/admin/verify/reports/diff?before=&after=` — **두 실행을 맞대어** 규칙별 `before`·`after`·`delta` 와 판정 변화를 낸다. `/reports/latest` 는 한 실행이라 *"원래 0건"* 과 *"고쳐서 0건"* 이 구분되지 않는다. **총합만 내면 한 규칙이 줄고 다른 규칙이 같은 수만큼 는 경우가 '변화 없음' 으로 보인다** — 그래서 규칙별로 낸다. 비교 불가는 0 이 아니라 거절이다: `dataset`·`scope` 불일치와 같은 실행은 400 `VERIFICATION-025`, 아직 판정이 없는 실행은 **409** `VERIFICATION-026`(파라미터가 아니라 상태라 같은 번호로 다시 부르면 된다). 시드가 심은 행은 `origin='BATCH'` 게이트에 걸려 404 다 — 안 걸면 *"배치가 800건을 고쳤다"* 는 거짓 증거가 나온다 |
| **했다 (CY-947)** | `GET /api/v1/admin/verify/reports/residual?before=&after=` — 같은 둘을 대상 키로 맞대 **규칙별 건수**로 접어 `persisted`·`introduced`·`resolved` 를 낸다. **`diff` 로는 못 가르는 것이 있다**: `STOCK_MISMATCH 3 → 3` 이 *"그 3건이 그대로"* 인지 *"3건 고쳐지고 새로 3건"* 인지 응답이 같은 모양이고, 앞엣것은 아무도 안 고치고 있는 것이고 뒤엣것은 지금도 계속 깨지고 있는 것이라 **처방이 정반대다**. `remaining = persisted + introduced` 가 사전예약 PRD 대사 보고서의 **잔여 불일치 건수**다. 가드는 `diff` 와 같은 것을 그대로 지난다(같은 실행 400, 미종료 409, 시드 행 404). 집계는 **DB 가 접는다** — 규칙당 상한 10,000 × 규칙 여섯이라 두 실행이면 최대 12만 키인데 이 조회 예산은 5초다. **실측(CY-949): 상한 12만 키에서 256~292ms — 예산의 6% 안쪽이다.** `uk_run_finding` 을 **커버링**으로 타고 `filesort` 는 없다(프로브가 단언한다). 표를 120k→720k 로 키우며 같은 대상을 여섯 번 쟀더니 **시간이 평평했다** — 표가 6배가 되는 동안 안 움직인다. ⚠️ 이 수는 **계약이 아니라 한 실행의 관측**이다(프로브가 재현하는 것은 실험이지 값이 아니다). ⚠️ **읽기 호출은 법칙을 못 세웠다**(비율에 따라 오르내리고 같은 형상도 재현이 안 된다) — 그래서 프로브가 그 축에 단언을 안 건다. 이 자리에 **두 번 틀린 문장을 적었다** — *"IN 두 값이 각각 구간으로"*(CY-947), *"비용은 대상 행 수를 따른다"*(CY-949 1차). 둘 다 재고 나서 뒤집혔다. `(finding_type, target_key)` 인덱스는 **안 만든다** — 마이그레이션이라 시드 DDL 과 함께 가야 하는데 300ms 안쪽으로는 살 이유가 없다. 규칙당 상한이 바뀌면 `ResidualQueryCostProbe` 가 빨개진다 — 그 값을 파일에서 읽어 대조한다. 판정은 한 키가 한 실행에 두 번 못 나오는 성질로 `sides` 1/2 를 가르고, 키 비교는 **바이트 기준**이다 — 기본 콜레이션은 대소문자를 무시해 다른 검출을 한 키로 합친다. **그 유니크가 사라지면 지속을 과대 보고한다. 🔁 **대상 목록은 이 응답에 없다** — 그 축은 CY-954 가 별도 조회로 낸다 |
| **했다 (CY-954)** | `GET /api/v1/admin/verify/reports/residual/targets?before=&after=&kind=&cursor=&limit=` — 잔여·신규·해소를 **대상 단위**로 한 페이지씩 준다. 요구사항 FR-C-11 이 *"건수뿐 아니라 예약·유형을 비교"* 라 적었고, 집계만으로는 화면이 *"3건 남았다"* 까지만 알고 **어느 회차인지 모른다**. **집계를 이것으로 대신하지 말 것** — 접힌 집계는 12만 키에서 6행이고 이 목록은 최대 12만 행이다. ⚠️ **`LIMIT` 이 비용을 거의 안 줄인다**(실측: 100건 220ms · 전량 270ms) — `GROUP BY` 가 임시 테이블로 집합을 다 만든 뒤에 자르기 때문이다. 그래서 커서를 **파생 테이블 안쪽**에 밀어 넣어 뒤 페이지가 싸지게 했다(맨 앞 237ms · 중간 52ms · 거의 끝 19ms). **바깥으로 옮기면 결과가 같아서 정확성 시험이 하나도 안 깨진 채 조용히 느려진다** — `ResidualTargetCostProbe` 가 읽기 호출 비율로 그 자리를 지킨다. ⚠️ `kind` 는 **시간을 안 줄인다**(`HAVING` 이라 집합은 어차피 다 만들어진다: 231ms → 193ms) — 줄어드는 것은 바이트와 페이지 수다. 페이지 상한 1,000 은 **바이트에서 나온 값**이다 — 행당 70B(짧은 키)~89B(긴 키)라 한 응답이 70~89KB 다. 상한 없는 전량은 **12만 행 · 8.4~10.7MB** 다(규칙당 10,000 × 6종 × 두 실행, 겹침 0). ⚠️ 한때 여기 31.5B/행(전량 2.7MB)이 적혀 있었는데 **프로브가 응답이 아니라 DB 행 텍스트를 셌던 것**이다. 커서는 **Base64URL 불투명 토큰 하나**다 — `DUP_PER_MEMBER` 의 대상 키가 `COUPON:1|MEMBER:2` 인데 **`|` 는 톰캣이 요청 타깃에서 거부하고**(기본 설정), 그 유형은 정렬상 맨 앞이라 **첫 페이지 커서**가 바로 그 모양이다. 거부는 컨트롤러에 닿기 전이라 봉투도 코드도 없이 400 이 나간다. 지어낸 토큰은 `VERIFICATION-027` 로 거부한다(조용히 처음부터 주면 부르는 쪽이 **앞 페이지를 다시 처리한다**). `CAST(... AS BINARY)` 는 `GROUP BY`·`ORDER BY`·커서 셋 다 있어야 한다 — ⚠️ 다만 **바깥 `ORDER BY` 만은 시험이 못 지킨다**(빼도 결과가 안 바뀐다 — 임시 테이블 집계라 그룹 키 순으로 나온다. MySQL 이 보장하는 순서는 아니다). **후속 조치는 없다** — 조치 범위는 Q-14 미정이고, 정해지기 전에 자리를 만들면 그 자리가 결정을 대신한다. ⚠️ 위 수는 **한 기기의 한 실행**이다 — 예산 근거로 쓰지 말 것 |
