...
위 `~0.2 ms` 는 **INSERT 셋일 때** 잰 것이다. CY-976 이 알림·아웃박스 INSERT 둘을 같은 트랜잭션에 더했고(§9.6), 늘어난 폭은 **안 쟀다.** fsync 는 트랜잭션당이라 커밋 비용은 그대로다.
...
사전 `SELECT … FOR UPDATE` 는 제거된다. 다만 Redis 선점 뒤 발급 트랜잭션 안에서 `active_count`를 1 증가시킨다(§9.6). 따라서 영속화 단위는 **다섯 테이블 INSERT와 카운터 UPDATE**다 — `issuances` · `issuance_histories` · `idempotency_records` · `notifications` · `notification_outbox`.

뒤의 둘은 CY-976 이 더한 것이다(#326 — v2 가 알림을 한 건도 안 만들고 있었다). 커밋 비용은 안 변한다 — §10 이 적듯 fsync 는 행이 아니라 **트랜잭션**에 붙는다. 다만 SQL 실행 구간은 늘었고 **그 폭은 안 쟀다.** §10 의 `~0.2 ms` 와 그것을 근거로 한 TPS 는 INSERT 셋일 때의 수치이므로, 이전에 잰 v2 성능 수치와 지금 재는 수치를 그대로 견주면 안 된다.

**v1 과 배타 실행이어야 한다.** 같은 회차를 동시 처리하면 v1 의 행 잠금이 Redis 카운터를 모른 채 재고를 깎는다. 버전 전환은 회차 단위 토글로 하고, 전환 시점에 §6.2 재구성을 한 번 태운다.

**엔진이 확정되는 시점은 워밍업이다.** 토글(`issuance_engine_version`)은 `issuance_engine_locked`
가 거짓인 동안에만 바뀌는데, **그 창이 워밍업이 도는 창과 정확히 같다**(둘 다 `NOW < open_at`).
그래서 워밍업이 잠그지 않으면, 게이트를 연 회차가 그 뒤 V1 으로 되돌아갈 수 있다 — 그러면 v1 이
Redis 를 아예 안 만져서 `ACTIVE_DB_GAP` 이 계속 CRITICAL 인데 **어느 지표도 "엔진이 바뀌었다" 를
말해 주지 않는다.** 그래서 §6.2 의 워밍업이 `meta` 를 쓰기 전에 `issuance_engine_locked` 를 함께
잠근다. 발급 경로(`lockAndFindById`)가 잠그는 것은 그때까지 아무도 안 잠갔을 때의 뒷수습이지
소유권이 아니다.

**대가 — 워밍업 뒤에는 오픈 전이라도 V1 으로 못 되돌린다.** "오픈 전·마감 후에는 `UPDATE` 허용"
이라는 창이 **워밍업 시점까지로 줄어든다.** 그게 의도다: 워밍업이 곧 확정이다. 되돌려야 하면
`meta` UNLINK 와 `issuance_engine_locked` 해제를 **손으로** 한다 — 프로토타입에서는 코드로
만들지 않는다.
...
**자가 치유** — `-7 REPLAY_PENDING` 인데 DB 에 발급이 없으면 그 자리에서 INSERT 를 다시 시도한다. 영구 손실이 재시도 한 번으로 회수된다.

**재시도는 원 요청 자신이 아니다.** 같은 멱등키라도 요청토큰이 다르다(§4.4). 원 실행이 아직 INSERT 를 커밋하는 중일 수 있고, 그 사이 재시도가 "발급 없음"을 보고 INSERT 를 또 넣을 수 있다. **이 경합을 막는 것은 `uk_coupon_member`(`issuances(coupon_id, member_id)` UNIQUE) 다** — 두 번째 INSERT 가 DB 에서 튕긴다. 재시도는 그 실패를 `-7` 로 되돌리면 되고, 완료 CAS 는 토큰이 다르므로 원 실행 것만 통과한다. 애플리케이션 조건 검사에 기대지 않는다.
...
| 회수 지점 | 왜 안전한가 |
|---|---|
| `-7` 재시도의 자가 치유(§4.6) | 원 실행과 겹쳐도 **`uk_coupon_member` 가 중복 INSERT 를 튕긴다** |
...
**워밍업을 부르는 것은 batch 의 내부 API 다.** 스케줄러가 아니다 — 주기 스캔은 "이미 열린 회차를
다시 만나는" 경우를 반드시 만드는데, 그 경우의 정답(게이트를 닫고 재구성)은 이 단위의 범위 밖이다.
회차 오픈 전에 **한 번** 부르면 된다(§10.2 는 단계마다 새 회차를 쓴다).

```
POST http://batch:9091/internal/v1/coupon-rounds/{r}/warmup
  200 WARMED               올라갔다. 응답 본문의 집계 넷을 그 자리에서 대조한다
  409 GATE_ALREADY_OPEN    meta 가 이미 있다. 아무것도 안 썼다 — 다시 만드는 것은 별개 절차다
  409 ENGINE_NOT_V2 / ROUND_ALREADY_OPENED / STOCK_ROW_MISSING / OVER_ISSUED_ROUND
  404 ROUND_NOT_FOUND
```

**안 부르면 그 회차 발급은 전량 `-9` → 503 이다.** 회차를 만드는 절차와 이 호출은 한 쌍이다.
9091 은 compose 에서 공개하지 않으므로 내부 네트워크에서만 닿는다.
