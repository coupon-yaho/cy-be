...
만료는 세 지점에서 처리합니다. 2·3계층이 있어서 1계층의 지연이 사용자에게 무해합니다.

| 계층 | 하는 일 | 소유 |
|---|---|---|
| 1. 배치 | `expires_at < now` 인 ISSUED를 EXPIRED로. **재고 +1 복원** | ④ |
| 2. 사용 시점 | 배치가 아직 안 돌았어도 사용을 막음 → `409 COUPON_EXPIRED` | ① |
| 3. 조회 시점 | 쿠폰함에서 `expires_at < now` 면 만료로 표시 | ① |

**만료 지연은 결함이 아닙니다.** 2·3계층이 사용자를 보호하므로 지연의 유일한 영향은 "재고가 아직 안 돌아왔다"뿐이고, 그건 배치 주기의 함수입니다. 검증 배치도 이력 재구성으로 이 상태를 오탐하지 않습니다 — 이력에 EXPIRE 이벤트가 없으면 재구성 상태도 ISSUED라 현재 상태와 일치합니다.
...
| # | 오염 유형 | 건수 | 잡는 규칙 | 위반 단위 |
|---|---|---|---|---|
| 1-a | 쿠폰 행은 있는데 history에 ISSUE 기록 없음 | 50 | V3 `ORPHAN_COUPON` | 쿠폰 |
| 1-b | 쿠폰 행 없이 재고 카운터만 틀림 | 50 | V1 `STOCK_MISMATCH` | 캠페인 |
| 2 | history는 USED인데 coupon.status는 ISSUED | 100 | V3 `REPLAY_MISMATCH` | 쿠폰 |
| 3 | CANCEL_USE가 2번 기록되어 재고 이중 복원 | 100 | V4 `ILLEGAL_TRANSITION` | 이력 행 |
| 4 | 종단 상태(EXPIRED)에서 USED로 불법 전이 | 100 | V4 `ILLEGAL_TRANSITION` | 이력 행 |
| 5 | 동일 쿠폰이 두 유저에게 발급됨 | 100 | V7 `DUPLICATE_CODE` | code 쌍 |
| 6 | 동일 유저가 같은 캠페인에서 2건 발급 | 100 | V2 `DUP_PER_MEMBER` | (캠페인, 회원) |
| 7 | status=ISSUED인데 canceled_at IS NULL usages 잔존 | 100 | V5 `USAGE_MISMATCH` | 쿠폰 |

유형 7은 `02-erd-decisions.md` F3에서 규칙 V5(사용 실적 정합)와 함께 추가됐습니다. 오염 유형이 늘어나는 건 검증이 강해진다는 뜻이라 부담이 아닙니다.

오염을 심는 단위와 검출하는 단위를 유형마다 일치시켰습니다

유형 1을 1-a와 1-b로 쪼갠 이유가 여기 있습니다. 원문 "재고는 줄었는데 history에 ISSUE 기록 없음"이 두 가지로 읽히는데, (a) 쿠폰 행은 있고 이력만 없는 것은 **실제 버그 클래스**이고 쿠폰 단위로 잡힙니다. (b) 쿠폰 행 없이 카운터만 틀린 것은 인공 오염이지만 **V1(재고 정합)을 오염셋으로 검증하는 유일한 방법**입니다. V1은 초과 발급 판정의 핵심 규칙이라, 오염 유형이 없으면 "빈 규칙이어도 700건 통과"가 됩니다.

1-b는 서로 다른 50개 캠페인에 하나씩 심습니다. V1이 캠페인 단위 집계라 한 캠페인에 여러 개를 심으면 findings가 하나로 뭉치고, 두 유형을 같은 캠페인에 심으면 서로 반대 방향으로 카운터를 틀어 **상쇄되어 0건**이 나옵니다.

합격 기준

오염 셋에서 정확히 700건 검출 AND 정상 셋에서 0건. `검증 결과 0건`만으로는 검증이 잘된 건지 아무것도 안 보는 건지 구분할 수 없습니다. 오염셋이 그걸 가르는 유일한 방법입니다.

판정은 건수 비교가 아니라 집합 비교입니다

`오염 주입 Job이 산출한 expected_findings`와 `verification_findings`를 양방향 `MINUS`로 대조합니다. 누락 0건 AND 오탐 0건이어야 합격입니다. **건수만 세면 오탐 350 + 미검출 350도 700입니다.**
...
``
| POST | /api/v1/admin/verify/runs/{runId}/stop | 실행 중단 (JobOperator.stop) |
``
...
```
campaign_stats(run_id, campaign_id, issued, used, cancelled, expired, sold_out_seconds_from_open)
grade_stats(run_id, campaign_id, grade, issued, used)
hourly_stats(run_id, day_of_week, hour, issued)
```
...
검증 도중 중단되면 세 테이블이 절반만 갱신된 채 남고, 대시보드가 이번 실행의 부분값과 직전 실행의 잔여값을 섞어 읽습니다. run_id를 붙이면 **완결되지 않은 스냅샷은 물리적으로 조회되지 않습니다**.

```sql
CREATE VIEW v_latest_stats_run AS
SELECT id FROM verification_runs
WHERE dataset = 'CLEAN' AND stats_status = 'COMPLETE'
ORDER BY as_of DESC LIMIT 1;
```

모든 통계 쿼리가 이 뷰를 씁니다. 화면이 늘어날 때 필터를 빠뜨릴 수 없게 하기 위해서입니다. 저장 부담은 없습니다 — run 100번이 쌓여도 총 9만 행입니다.

**제출 직전 마지막 실행은 반드시 CLEAN이어야 합니다.** 오염 run으로 끝내면 `stats_status=SKIPPED`라 뷰가 이전 run을 가리키고, 통계 차트가 조용히 옛날 데이터를 보여줍니다.
...
마지막 3열(초과 · 1인초과 · 불일치)의 산출 주체는 ④입니다

부하 종료 후 L2에서 검증 배치를 1회 돌려 채웁니다. 나머지 열은 k6(②⑤)가 채웁니다. **소진 시간 열은 k6 값만 씁니다** — campaign_stats의 sold_out_seconds_from_open은 기점(open_at)과 종점(DB 이력 시각)이 둘 다 달라서 v3에서 정반대 방향으로 벌어집니다. 그건 과거 캠페인 랭킹 전용입니다.
...
| ① 도메인·코어 | 엔티티/ERD, 상태 전이, 멱등성, 브랜드·스케줄·스케줄러, 등급 검증, 만료 2·3계층, 실시간 드리프트 감지 | 도메인 모듈, 스케줄러, API 스펙 |
...
| ④ 데이터·검증 | 더미데이터 생성기, 검증 배치, 오염셋 700건, 리포트, 통계 집계, **만료 배치(1계층)**, 정리·DLT 재처리 | 시드 모듈, 검증·집계 배치 |
...
| ① | 만료 3계층 중 2·3계층(사용 시점 409 · 조회 시점 판정) + 브랜드 데이 스케줄러 + 실시간 드리프트 감지 |

| ② | v2 Redis Lua 완성 + k6 시나리오 분담 |
``
| ③ | v3 Kafka 비동기 + 대기열 로직을 /entry에 탑재 + CB 통합 |

| ④ | 검증 배치 300만 전수 + 오염셋 700건 + 리포트 + 통계 집계 테이블 + 만료 배치 |

| ⑤ | 대시보드 폴링 연결 + 이벤트 스트림 + 관리자 CRUD + Chaos 하네스 |

⚠️ D8 판단 지점v3가 막히면 v2까지로 축소하고 v3는 설계·PoC 발표로 전환. D11 이후로 미루지 않는다.

D10 게이트v1·v2가 20,000 풀스케일 통과 · v3 완성 · 검증 배치가 오염셋 700건 정확 검출 · 대시보드 실시간 동작 · 스케줄러가 캠페인 자동 생성
...
``
| 14 | 만료 배치 실행 중 같은 쿠폰에 cancel/use 동시 | 재고 1회만 복원, 상태 1회만 전이 |

14번을 5번과 따로 두는 이유

5번(발급 + 만료 배치)은 재고를 **반대 방향**으로 움직여 충돌이 눈에 띕니다. 반면 만료와 취소는 **같은 방향**(둘 다 복원)이라 이중 적용이 조용히 통과합니다. 오염 유형 3(CANCEL_USE 이중 복원)이 겨냥한 결함 클래스와 정확히 같습니다.

방어는 조건부 UPDATE입니다 — `UPDATE coupons SET status='EXPIRED' WHERE id=? AND status='ISSUED'` 의 affected rows를 세어 그 수만큼만 감산합니다. SELECT로 읽은 건수로 감산하면 그 사이 취소된 쿠폰까지 세어 재고가 이중 복원됩니다.

5번과 14번 모두 소유는 ④입니다 — 만료 배치가 ④로 이관됐기 때문입니다.
...
| 3:30–5:00 | 고의 오염 검출 — 정상 셋에서 0건, 오염셋에서 700건 정확 검출 | 검증이 작동함을 증명 |
...
| 검증 배치가 0건이면 정말 문제가 없나요? | 그 질문 때문에 오염 데이터셋 700건을 만들었습니다. 주입 시 산출한 정답 목록과 집합 비교해 누락 0·오탐 0이므로 "0건"이 신뢰할 수 있는 0건입니다 |
...
| R8 | 캠페인 147개가 검증에 파급 | D6 검증 쿼리에 캠페인 루프 발생 | 검증 쿼리를 캠페인 루프 없이 단일 GROUP BY로 쓰는 것을 제약으로 못박음 | 과거 캠페인 48개로 축소, 캠페인당 재고 증대 | ④ |
...
| A3 | 브랜드 데이 추가가 동시성·검증에 파급 안 됨 | 검증 쿼리가 캠페인 루프 없이 도는지 확인 | D6 | 과거 캠페인 48개로 축소 |
...
``````````````
| verification_runs | as_of · dataset · scope · attempt · verdict · stats_status · findings_checksum · dataset_fingerprint | 검증 실행 이력. verdict(규칙 기준)와 stats_status를 한 컬럼에 섞지 않음 |
````````````
| verification_findings | run_id · finding_type · campaign_id · member_id · coupon_id · history_id · expected · actual | 검출 항목. 규칙마다 위반 단위가 달라 다형 키가 필요함 |
``
| expected_findings | seed_run_id · finding_type · 다형 키 · corrupt_type | 오염 주입이 산출하는 정답 매니페스트. CORRUPT 스키마에만 |
``
| asof_state / asof_state_meta | 쿠폰별 asOf 시점 상태 · 생성 시점 지문 | 이력 리플레이 산출물. 지문이 같으면 다음 run이 재사용 |
``
| campaign_stats | run_id · issued · used · cancelled · expired · sold_out_seconds_from_open | 브랜드 전환율 · 소진 랭킹 차트 |
``````
| grade_stats | run_id+campaign_id+grade PK · issued · used | 등급별 분포 차트 |
``````
...
배치 서버 — 부하 중에는 내려둠. 종료 후에만 기동
```

배치 서버는 별도 compose 파일로 분리합니다

`docker compose -f base.yml up` 으로 부하를 재고, 종료 후 `-f base.yml -f batch.yml up batch` 로 검증만 올립니다. **base.yml은 한 글자도 안 바뀌므로 "동일 Docker Compose 리소스 limit"이 유지됩니다.**

부하 중에 배치를 돌리면 측정이 오염되지만, 부하가 끝난 뒤에는 오염될 측정이 없습니다. 그리고 Chaos 통과 기준("장애 종료 후 검증 불일치 0건")과 불변식 판정("부하 종료 + Kafka 백로그 소진 후 DB 기준")은 **L2 DB에 대해 검증 배치를 돌려야만** 자동 판정됩니다.
