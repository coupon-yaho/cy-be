...
| 캠페인 수 | 구간 시작·종료 시점의 활성 캠페인 수, 그리고 구간 중 캠페인이 닫힌 시각. 캠페인 미터는 `coupon_id` 라벨을 쓰므로 페이로드가 여기에 비례해 계단을 만든다 |
...
없다. 실제로 이 때문에 없는 결함을 하나 만들어 낸 적이 있다 — 캠페인 미터가 사라진 것으로
...
# 페이로드의 시간 변화 — 캠페인 수 변동이 여기 계단으로 보인다
...
**부하 중 vs 유휴를 같은 스택에서 비교한다.** 페이로드 구성은 그대로 두고 CPU 경합만
달라지므로, 차이가 나면 그 몫은 코어 경합의 것이다.
