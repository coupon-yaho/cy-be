...
절차는 `docs/14` 에 있다. **셋 다 기동 가드가 잡는다**(CY-686 이 인덱스 축을 더했다) — 테이블·컬럼·인덱스
세 축이고, 인덱스는 이름뿐 아니라 선두 컬럼까지 대조한다. 인덱스 축만은 정확성이 아니라 성능이라
`batch.schema-guard.require-batch-indexes=false` 로 거절을 끌 수 있다 — 그때는 ERROR 로그로 남는다.
...
`/api/v1/admin/**` 에는 트리거만이 아니라 `stop`·`recover`·`abandon` 이 있어 **포트에 닿는
누구나 도는 잡을 죽일 수 있다.**

**이것은 위 결정과 모순이 아니다.** 여기서 거부한 것은 서명 없는 **역할 클레임**이다 —
클라이언트가 *주장*하는 값이라 방어가 아니라 장식이다. 토큰 관문은 주장이 아니라 **소지**를
묻는다: 값을 **가진** 쪽만 통과한다. JWT·세션·Spring Security 는 그대로 안 들였다.

**충분하지 않다는 것도 같이 적는다.** TLS 가 없으면 평문으로 오가고, 유출되면 회수 수단이
재기동뿐이다. **1차가 여전히 1차**이고 2차는 그 뒤에 선다 — `batch-expose.yml` 을 얹을지는
토큰 관문이 있다고 해서 자동으로 답이 나오지 않는다.

**켠 채 토큰이 비면 기동을 거절한다.** "관문 있는 척" 이 제일 나쁘다. 일부러 열어 둘 때는
`BATCH_ADMIN_AUTH_REQUIRED=false` 로 명시하고, 그 상태는 `BatchAdminAuthDisabled` 알림이
15분 뒤 운다.
