...
```
Step 0 asof_state 생성  +  V4          접기 한 번에 산출물 둘
────── 완전 결정론 ──────
Step 1 V5   Step 2 V2
────── 현재 행을 읽음 ──────
...
### 오염 주입은 의도한 규칙 하나만 울려야 한다

**시드가 지켜야 하는 계약이다.** 규칙이 축을 여럿 보므로, 한 축만 비틀고 나머지를 그대로 두면
의도하지 않은 규칙이 함께 운다. 그러면 양방향 MINUS 에서 **오탐**으로 잡혀 게이트가 떨어진다.

계약(`docs/contract.json`)의 `corruption` 은 *무엇을 몇 건 기대하는가*만 적는다.
**어떻게 심어야 그것만 나오는가는 여기 있다.** 시드를 다시 구현할 때 이 표를 먼저 본다.
...
| 3 사용취소 이중 기록 | `CANCEL_USE` 이력 하나 추가 | 최종 상태가 `USED` 로 유지되게 뒤에 `USE` 를 둔다. 활성 사용 1건 유지 | 최종 상태가 `ISSUED` 로 굳으면 `V3` 가, 활성 사용이 어긋나면 `V5` 가 함께 운다 |
| 4 종단에서 되살림 | `EXPIRED → USED` 이력 하나 추가 | **`issuances.status` 를 `USED` 로 맞추고 활성 사용 행을 1건 넣는다** | 접기가 마지막 `to_status` 를 따라가 `USED` 가 되므로, 둘 다 안 맞추면 `V3` 100건 + `V5` 100건이 함께 운다 |
| 5 코드 중복 | 같은 `code` 를 두 회원에게 | 회차·상태·사용 축은 정상 그대로 | — |
| 6 1인 2매 | 같은 회원에게 두 건 | 회차·상태·사용 축은 정상 그대로 | — |
...
**`issuances.updated_at` 은 `asOf` 이하여야 한다.** 주입하면서 `NOW()` 로 찍으면 배치가
실행 시작에 거부한다 — `V3` 가 그 행들을 비교에서 빼면 기대 100건이 조용히 0건이 되기 때문에,
빼는 대신 멈춘다.
