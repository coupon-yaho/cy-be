...
#### ⚠️ 아직 확인 못 한 것 — 설정을 어느 브랜치에서 읽나

`.pr_agent.toml` 을 만들어 `response_language = "ko-KR"` 을 넣었는데, **PR head 브랜치에
올려 놓고 돌린 리뷰가 영어로 왔다.** 원인 후보가 둘이다.

| | |
|---|---|
| ⑴ **base 브랜치에서 읽는다** | `.coderabbit.yaml` 이 `main`·`CY-1`·`CY-5`·`CY-15` **네 곳 모두**에 있는 것이 그 증거다. 그렇다면 이 파일이 `feature/CY-15` 에 올라간 뒤부터 먹는다 |
| ⑵ **키 이름이 다르다** | 그때는 계속 영어다 |

**가르는 법** — 머지 뒤 새 PR 에서 리뷰가 한국어로 오면 ⑴, 여전히 영어면 ⑵ 다.

⑴ 이면 **다른 에픽 브랜치(`CY-1`·`CY-5`)와 `main` 에도 같은 파일을 넣어야 한다** —
그 브랜치를 base 로 삼는 PR 들은 지금 설정 없이 돈다. `.coderabbit.yaml` 이 네 곳에 있는
것과 같은 이유다. **그건 다른 영역 담당자들 PR 에도 영향이라 팀에 알리고 해야 한다.**
