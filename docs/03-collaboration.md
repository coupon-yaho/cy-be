...
#### ⚠️ `.pr_agent.toml` 은 **기본 브랜치(main)** 에 있어야 한다

Qodo 의 저장소 설정 절차가 그렇게 명시한다. **에픽 브랜치에만 머지하면 안 먹는다** —
한국어도 규약도.

> **여기서 한 번 틀렸다.** `.coderabbit.yaml` 이 `main`·`CY-1`·`CY-5`·`CY-15` 네 곳에 모두
> 있는 것을 보고 *"리뷰어는 base 브랜치에서 설정을 읽는다"* 로 추정했는데, **그것은
> CodeRabbit 의 동작이지 Qodo 의 것이 아니다.** 상관을 근거로 단정했고, 그 오판을
> **Qodo 자신이 첫 한국어 리뷰에서 짚었다.**

| | 설정을 읽는 곳 |
|---|---|
| CodeRabbit | PR 의 **base 브랜치** — 그래서 네 브랜치에 사본이 있다 |
| **Qodo** | **기본 브랜치(`main`) 루트** 하나 |

**그래서 이 파일을 `main` 에 올려야 팀 전체 PR 에 적용된다.** 지금은 `feature/CY-15` 로
가는 중이라, 머지돼도 다른 영역(`CY-1`·`CY-5`)의 PR 은 설정 없이 돈다.
**다른 담당자들에게 영향이라 팀에 알리고 해야 한다.**

**언어 키는 확인됐다** — `[review_agent].issues_user_guidelines` 를 넣자 리뷰가 한국어로 왔다.
`[config].response_language` 만으로는 영어였다. `[pr_reviewer].extra_instructions` 는
오픈소스 PR-Agent 용이라 호스팅 Qodo 에서는 안 쓰이는 것으로 보인다.
