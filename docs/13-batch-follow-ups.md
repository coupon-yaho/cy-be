...
| `verifyJob` 의 `stop` 에 시체 판정이 없다 | **닫혔다(CY-678).** 기본은 진도가 멈춘 실행만 받고(409 `VERIFICATION-019`), 판정과 쓰기를 `VerifyStopService` 의 선점 `UPDATE` 로 한 트랜잭션에 묶었다. 도는 검증을 정말 세워야 할 때는 `?force=true` 로 연다 — 잡 전체 데드라인이 없어 `replayStep` 은 영원히 시체가 안 되고, 못 세우면 만료가 `updated_at` 을 찍어 그 `asOf` 가 죽는다 |
