...
| ↑ ~~**CY-421 이 그 축을 알림 경로로 넓혔다**~~ **좁혔다 · CY-768** | ⚠️ **이 칸이 오래 과장돼 있었다.** *"실행이 끝난 뒤 `CANCEL_USE` 로 되돌아온 행이 새로 세어진다"* 는 일반적으로 성립하지 않는다 — `CouponStateMachine.cancelUse` 가 **만료 뒤 취소를 `EXPIRED` 로** 보내므로 `COUNT_PENDING`(`expires_at < asOf`)에 안 걸린다. `ISSUED` 로 돌아오려면 `claimedAt ≤ expires_at < asOf ≤ 실행 < 취소의 커밋` 이어야 하고, 즉 **취소의 선점→커밋 간격이 (asOf − expires_at)보다 커야** 한다. 즉 그 간격이 클수록 오래된 만료 건까지 닿는다. ⚠️ **"그러니 불가능하다" 로 쓰면 안 된다** — 재 보니 **취소 트랜잭션 전체에는 상한이 없다**(`CouponCancelUseService.cancelUse` 의 `@Transactional` 에 timeout 속성이 없다). 있는 상한은 문장 단위뿐이고, **그 값들도 배포 보장이 아니라 계층에서 온다** — 출처를 밝혀 적는다: 문장 왕복 `socketTimeout` 의 **공통 기본값은 60,000ms** 다(`storage.yml.example`). 취소는 api 에서 도므로 이 경로에 걸리는 값이 그것이다. **660,000ms 는 배치 전용 override** 이고(`batch.yml`, `replayStep` 312초 때문) api 에는 안 걸린다. ⚠️ 한때 여기 660,000ms 를 공통값이라고 적었는데, 내 로컬 `storage.yml` 을 보고 쓴 것이었다 — 그 파일은 `.gitignore` 대상이라 저장소 기본이 아니다. 둘 다 `DB_SOCKET_TIMEOUT_MS` 로 덮인다. 락 대기 `innodb_lock_wait_timeout=50s` 와 유휴 `wait_timeout=28800s` 는 **compose 가 지정하지 않아 MySQL 이미지 기본값**이고, 위 숫자는 로컬 컨테이너에서 읽은 것이다 — 배포 서버가 다르면 달라진다. 그리고 문장 **사이**의 GC 정지·스레드 지연에는 제한이 없다. **빈도는 안 쟀다** — `docs/12` §8 이 *"동시 부하 형상은 여전히 안 쟀다"* 고 적는다. 그러니 여기서 말할 수 있는 것은 *"불가능하지 않다"* 까지이고, 어떤 형상이 흔한지는 그 측정이 생긴 뒤에 적는다. CY-768 이 이력 id 상한으로 그 창을 더 좁혔지만 **닫지는 못한다**: AUTO_INCREMENT 할당 순서와 커밋 순서가 달라(실측) id 를 먼저 받고 경계 읽기 뒤 커밋하는 취소는 안 걸린다. 경계를 일찍 읽으면 실행 중 변경을 숨기고 늦게 읽으면 이 경합이 생겨 **경계로는 여기까지가 끝**이다 — 커밋 순서 축(binlog·GTID)이거나 잡이 자기 결과를 남기는 방식으로 되돌려야 한다(CY-421 결정을 되짚는 일이라 별개 티켓) |
...
| 스케줄 작업이 풀보다 많아도 그냥 뜬다 | **닫혔다.** `SchedulerPoolGuard` 가 기동 때 `ScheduledAnnotationBeanPostProcessor.getScheduledTasks()` 로 **실제 등록 수**를 세어 `spring.task.scheduling.pool.size` 보다 크면 거절한다. **애노테이션만 grep 하면 수가 안 맞는다** — `SchedulingConfigurer` 로 등록되는 것이 따로 있어 그 API 로 센다. 안 막으면 증상이 고약하다: 검증이 한 번에 8분을 잡고 있는 동안 되읽기가 스레드를 기다려 게이지가 그만큼 낡는데, **배치는 멀쩡하므로 SLA 알림만 울고 `cy_batch_refresh_failures_total` 은 안 오른다.** 기본값은 `BATCH_SCHEDULER_POOL_SIZE:12` 이고 `batch.yml` 도 12 다(CY-946 이 `StuckRunSweeper` 를 더해 열둘이 됐다) — ⚠️ **그보다 앞서 구운 이미지는 10 이라 env 로 줘야 한다**(시연 준비에서 실제로 걸렸다) |
| 배치 URL 의 `socketTimeout` 이 Step 데드라인보다 짧아도 그냥 뜬다 | **닫혔다.** `DataSourceTimeoutGuard` 가 기동을 거절한다. 짧으면 **정상 배치가 매번 같은 자리에서 끊기는데 원인이 배치가 아니라 URL 에 있어** 찾기 어렵다 — `replayStep` 이 312초를 쓰므로 `socketTimeout` 60,000ms 와 `batch.verify.step-timeout-ms` 600,000ms 조합이 정확히 그 모양이다. 당장 띄워야 하면 `DATASOURCE_TIMEOUT_GUARD_REQUIRED=false` 로 거절만 끌 수 있다 |
...
### 3a. 인덱스 — 후보를 전부 재고 전부 기각했다 (CY-767)

**인덱스 제한이 풀린 뒤 후보를 하나씩 만들어서 쟀다. 넣을 것이 없었다.**
기각한 것도 숫자와 함께 남긴다 — 안 그러면 다음 사람이 같은 후보를 다시 판다.

전제: 300만 발급 · 534만 이력 · 132만 사용 · 버퍼 풀 128 MiB · `coupon_bench`.

| 후보 | 판정 | 실측 |
|---|---|---|
| `issuance_usages(issuance_id, used_at, canceled_at)` 커버링 | **기각** | 계획은 `Using index` 로 바뀌는데 전체는 48.5초 → **47.1초(3%)**. 집계 `SELECT` 자체는 1.06초 → 0.45초로 절반 이하가 되지만 **그 집계가 비용의 2%** 다. 98% 는 `asof_state` PK 무작위 갱신이라 인덱스가 안 닿는다. 게다가 인덱스가 120 MiB 를 더해 **버퍼 풀 병목을 키운다** — 병목이 버퍼 풀인데 워킹셋을 늘리는 처방이다 |
| `issuances(member_id, issued_at, id)` — 내 쿠폰함 | **불필요** | 이미 `fk_issuances_member` 로 `type=ref` · `rows=41` · **21ms**. 회원당 보유 최대가 41건이라 정렬도 41행짜리다 |
| `idempotency_records(created_at)` | **불필요** | `release` 도 `idem_key`(PK) 로 지운다. 정리 잡(`cleanupJob`)은 이 테이블을 안 걷는다 — 걷는 것은 `verification_runs`·`asof_state`·`BATCH_*` 뿐이고 그 인덱스는 `SchemaPresenceGuard` 가 이미 강제한다 |
| `issuance_histories` 추가 인덱스 | **불필요** | `(issuance_id, created_at, id)` 와 `(created_at, id)` 가 리플레이 창·정렬과 시각 집계를 이미 덮는다 |
| **replayStep 조인 순서 힌트** (`STRAIGHT_JOIN` + `FORCE INDEX`) | **기각** | 창 100,000 에서는 1.47초 → 0.50초로 **2.8배**였다. 그런데 **실제 창은 50,000**(`replay-window-size`)이고 거기서는 0.32 vs 0.30 으로 **차이가 없다**(콜드 구간 3회, 한 번은 힌트가 더 느렸다). ⚠️ 내가 임의로 잡은 창에서만 나던 값이다 — **파라미터를 실제 값으로 놓고 재야 한다** |
| `issuances(coupon_id, status, expires_at)` | **기각(기존)** | CY-742 실측. 리더는 선두가 `NOT IN` 이라 못 타고, 라이터는 이미 `rows=1` |

**패턴이 하나로 모인다** — 이 배치의 무거운 Step 은 전부 **쓰기이거나 버퍼 풀 바운드**다.
`replayStep`(45%)은 300만 `asof_state` 적재, `usageCountStep`(30%)은 192 MiB 워킹셋을
128 MiB 풀에 밀어 넣는 것이다. **읽기 계획이 병목인 Step 이 없어서 인덱스가 손댈 자리가 없다.**
줄이려면 인덱스가 아니라 **버퍼 풀**이다(12배, `docs/12` §usageCountStep).

⚠️ **다시 볼 기준** — 배포 버퍼 풀이 워킹셋(약 192 MiB)보다 커지면 병목이 옮겨 간다.
그때는 읽기 계획이 드러나므로 위 후보들을 **다시 재야 한다.** 지금 결론은
"버퍼 풀 128 MiB 에서" 라는 조건이 붙은 값이다.
...
종료 표시를 못 남기고 죽은 실행은 `STATUS` 조회에 `END_TIME` 검사도 시간 상한도 없다.
그동안 만료↔검증 상호 배제가 그 실행에 대해 꺼져 있다(CY-384).

> **⚠️ CY-946 부터 이 절은 1차 처방이 아니다.** `StuckRunSweeper` 가 60초마다 같은 판정으로
> `recover`(= `FAILED`)를 걸어 **사람 없이 걷는다.** 그래서 아래 절차가 필요한 경우는 둘뿐이다 —
> 스윕이 꺼져 있거나(`cy_batch_stuck_sweep_enabled == 0`), 배치가 안 떠 있어 스윕 자체가
> 안 도는 경우다.
>
> **⚠️ 스윕이 켜진 채로 아래 `stop → abandon` 을 쓰면 진다.** `stop` 이 성공하면 상태가
> `STOPPING` 이 되는데 그 실행의 하트비트는 영원히 안 움직이고, `STOPPING` 은 시체 판정과
> 선점문에 **둘 다 포함**된다 — 사람이 응답을 읽고 두 번째 호출을 쏘는 사이(보통 60초 초과)에
> 스윕이 먼저 `FAILED` 로 닫고 뒤이은 `abandon` 이 409 로 거절된다. **60초 안에 두 번째 호출을 쏘면 사람이 이긴다** — 그 폭은 안 쟀고, 재는 값도 아니다. **그것은 사고가 아니다** —
> `FAILED` 가 `ABANDONED` 보다 나은 결과이므로 그대로 두면 된다.
...
> 내리는 변경은 이 API 의 안전을 직접 깎는다. **CY-946 부터는 무인 조치 경로 전체를
> 깎는다** — 같은 임계가 스윕의 대상 선정도 구동한다.
...
> 그 행은 이제 `StuckRunSweeper` 가 먼저 닫고(CY-946), 스윕이 꺼진 형상에서만
> CY-429 의 복구 API 를 사람이 부른다 — 위 절차가 그것이다. **어느 쪽이든 `FAILED` 라
> 30일 뒤 정상 회수 대상이 된다** — 전에는 `END_TIME` 이 비어 있어 이 삭제가 영원히
> 안 걷던 행이다.
>
