...
| ↑ ~~**CY-421 이 그 축을 알림 경로로 넓혔다**~~ **좁혔다 · CY-768** | ⚠️ **이 칸이 오래 과장돼 있었다.** *"실행이 끝난 뒤 `CANCEL_USE` 로 되돌아온 행이 새로 세어진다"* 는 일반적으로 성립하지 않는다 — `CouponStateMachine.cancelUse` 가 **만료 뒤 취소를 `EXPIRED` 로** 보내므로 `COUNT_PENDING`(`expires_at < asOf`)에 안 걸린다. `ISSUED` 로 돌아오려면 `claimedAt ≤ expires_at < asOf ≤ 실행 < 취소의 커밋` 이어야 하고, 즉 **취소의 선점→커밋 간격이 (asOf − expires_at)보다 커야** 한다. 즉 그 간격이 클수록 오래된 만료 건까지 닿는다. ⚠️ **"그러니 불가능하다" 로 쓰면 안 된다** — 재 보니 **취소 트랜잭션 전체에는 상한이 없다**(`CouponCancelUseService.cancelUse` 의 `@Transactional` 에 timeout 속성이 없다). 있는 상한은 문장 단위뿐이다: 락 대기 `innodb_lock_wait_timeout=50s`, 문장 왕복 `socketTimeout=660s`, 유휴 커넥션 `wait_timeout=28800s`(= **8시간**). 문장 사이의 GC 정지·스레드 지연에는 제한이 없다. 그래서 긴 창은 **불가능이 아니라 관측상 드문 것**이고, 흔한 형상은 실행 직전에 만료되는 건이 긴 락 대기를 만나는 경우다. CY-768 이 이력 id 상한으로 그 창을 더 좁혔지만 **닫지는 못한다**: AUTO_INCREMENT 할당 순서와 커밋 순서가 달라(실측) id 를 먼저 받고 경계 읽기 뒤 커밋하는 취소는 안 걸린다. 경계를 일찍 읽으면 실행 중 변경을 숨기고 늦게 읽으면 이 경합이 생겨 **경계로는 여기까지가 끝**이다 — 커밋 순서 축(binlog·GTID)이거나 잡이 자기 결과를 남기는 방식으로 되돌려야 한다(CY-421 결정을 되짚는 일이라 별개 티켓) |
