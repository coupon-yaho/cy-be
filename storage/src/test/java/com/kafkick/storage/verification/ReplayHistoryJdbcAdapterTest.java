...
class ReplayHistoryJdbcAdapterTest {
    ...
    @Test
    @DisplayName("이력이 하나도 없으면 훑을 구간이 없다")
    void reportNoRangeWhenEmpty() {
        assertThat(adapter.issuanceIdRange(AS_OF)).isEmpty();
    }

    @Test
    @DisplayName("asOf 이하 이력을 가진 발급건의 양 끝을 준다")
    void reportRangeBounds() {
        long first = data.issuance(IssuanceStatus.ISSUED);
        long second = data.issuance(IssuanceStatus.ISSUED);
        data.history(first, IssuanceEventType.ISSUE, null, IssuanceStatus.ISSUED, AS_OF.minusHours(1));
        data.history(second, IssuanceEventType.ISSUE, null, IssuanceStatus.ISSUED, AS_OF.minusHours(1));

        Optional<IssuanceIdRange> range = adapter.issuanceIdRange(AS_OF);

        assertThat(range).contains(new IssuanceIdRange(first, second));
    }

    @Test
    @DisplayName("asOf 이후에만 이력이 있는 발급건은 구간에서 빠진다")
    void excludeIssuanceWithOnlyFutureHistory() {
        long visible = data.issuance(IssuanceStatus.ISSUED);
        long future = data.issuance(IssuanceStatus.ISSUED);
        data.history(visible, IssuanceEventType.ISSUE, null, IssuanceStatus.ISSUED, AS_OF.minusHours(1));
        data.history(future, IssuanceEventType.ISSUE, null, IssuanceStatus.ISSUED, AS_OF.plusHours(1));

        assertThat(adapter.issuanceIdRange(AS_OF)).contains(new IssuanceIdRange(visible, visible));
    }
    ...
    void readHistoriesInRange() {
        ...
        data.history(issuanceId, IssuanceEventType.ISSUE, null, IssuanceStatus.ISSUED, AS_OF.minusHours(2));
        data.history(issuanceId, IssuanceEventType.USE,
                IssuanceStatus.ISSUED, IssuanceStatus.USED, AS_OF.minusHours(1));

        List<IssuanceHistoryRecord> histories = adapter.findRange(issuanceId, issuanceId, AS_OF);
    ...
    void readNullFromStatus() {
        ...
        data.history(issuanceId, IssuanceEventType.ISSUE, null, IssuanceStatus.ISSUED, AS_OF.minusHours(1));
    ...
    void includeHistoryExactlyAtAsOf() {
        long issuanceId = data.issuance(IssuanceStatus.ISSUED);
        data.history(issuanceId, IssuanceEventType.ISSUE, null, IssuanceStatus.ISSUED, AS_OF);
    ...
    void excludeHistoryOutsideRange() {
        long inside = data.issuance(IssuanceStatus.ISSUED);
        long outside = data.issuance(IssuanceStatus.ISSUED);
        data.history(inside, IssuanceEventType.ISSUE, null, IssuanceStatus.ISSUED, AS_OF.minusHours(1));
        data.history(outside, IssuanceEventType.ISSUE, null, IssuanceStatus.ISSUED, AS_OF.minusHours(1));

        assertThat(adapter.findRange(inside, inside, AS_OF))
                .singleElement()
    ...
    void rangeBoundsAreInclusive() {
        ...
        assertThat(adapter.findRange(first, second, AS_OF)).hasSize(2);
    }
    ...
    void orderByIssuanceIdFirst() {
        ...
        long second = data.issuance(IssuanceStatus.ISSUED);
        data.history(second, IssuanceEventType.ISSUE, null, IssuanceStatus.ISSUED, AS_OF.minusHours(3));
        data.history(first, IssuanceEventType.ISSUE, null, IssuanceStatus.ISSUED, AS_OF.minusHours(2));
        data.history(first, IssuanceEventType.USE,
                IssuanceStatus.ISSUED, IssuanceStatus.USED, AS_OF.minusHours(1));

        assertThat(adapter.findRange(first, second, AS_OF))
                .extracting(IssuanceHistoryRecord::issuanceId)
    ...
    void readEmptyRange() {
        assertThat(adapter.findRange(1L, 10L, AS_OF)).isEmpty();
    }
    ...
    void preserveMicroseconds() {
        ...
        long issuanceId = data.issuance(IssuanceStatus.ISSUED);
        data.history(issuanceId, IssuanceEventType.ISSUE, null, IssuanceStatus.ISSUED, precise);
    ...
    void cutAtAsOfWithMicrosecondPrecision() {
        ...
        assertThat(adapter.findRange(issuanceId, issuanceId, asOf))
                .singleElement()
    ...
    void roundTripEveryStatus(IssuanceStatus status) {
        ...
        data.history(issuanceId, IssuanceEventType.ISSUE,
                status, status, AS_OF.minusHours(1));
