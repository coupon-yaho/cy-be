 ...
 * </pre>
 *
 * <p><b>재현되는 것은 실험이지 값이 아니다.</b> 프로브를 돌리면 <b>같은 여섯 형상</b>이
 * 다시 만들어지고 각 점의 수가 다시 찍힌다 — 그러나 <b>그 수가 위와 같으리라는 보장은
 * 없다.</b> 시간은 러너를 타고, 읽기 호출은 <b>같은 기기에서도 실행마다 달라진다</b>
 * (240k 에서 793,910 / 1,033,912, 720k 에서 673,910 / 1,393,912).
 * 위 표는 <b>한 기기의 한 실행에서 본 것</b>이다.
 *
 * <p>옵티마이저가 비율에 따라 계획을 갈아타는 것으로 <b>보이지만</b> 그 법칙을 세울 수는
 * 없다. <b>그래서 어느 수에도 단언을 안 건다</b> — 이 프로브가 안정적으로 지키는 것은
 * <b>계획의 모양</b>과 <b>상한에서의 정답</b>과 <b>상한 표류</b> 셋뿐이다.
 * 시간 평탄성은 <b>계약이 아니라 관측</b>이고, 그것을 계약으로 만들려면 러너에 안 흔들리는
 * 기준이 먼저 있어야 한다 — 지금은 없다.
 *
 * <p><b>결정에 쓰는 축은 시간이다.</b> 표가 <b>6배</b>가 되는 동안 <b>256~292ms 로
