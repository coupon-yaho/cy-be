 ...
 *
 * <p><b>재시도를 권하는 문구는 재시도로 풀리는 코드에만 쓴다.</b> {@link #REPLAY_PENDING} 과
 * {@link #GATE_NOT_READY} 둘뿐이고, 그 둘만 {@code Retry-After} 를 단다. 나머지 셋은 다시 눌러도
 * 같은 실패로 돌아오므로 문구로도 재시도를 권하지 않는다 — 권하면 사고 구간에 재시도가 겹친다.
 */
