...
/**
 * 캠페인이 끝났다는 사실을 관측 구현에 알리는 포트입니다.
 *
 * <p>{@link EventRecorder}와 섞지 않습니다. 이벤트 기록과 미터 수명 관리는 성격이 달라 한
 * 인터페이스에 두면 구현체 교체 단위가 엉킵니다.
 *
 * <p>이 통지는 {@code coupon_id} 라벨 예외의 대가입니다. 라벨을 열면 시계열이 캠페인 수만큼
 * 늘어나므로 언제 지울지는 도메인만 알 수 있습니다.
 */
public interface CampaignLifecycleRecorder {

    /**
     * 캠페인 종료를 통지합니다.
     *
     * <p>구현은 멱등이어야 합니다 — 같은 {@code campaignCouponId}로 여러 번 불려도 안전해야 종료 배치의
     ...
     *
     * <p><b>구현은 값 계약 위반을 호출자에게 전파하지 않습니다.</b> 이 통지는 캠페인 종료
     ...
     *
     * @param campaignCouponId 종료된 캠페인의 쿠폰 식별자. 양수여야 합니다.
     *                 <b>{@code coupons.id}(캠페인 회차)이며 {@code issuances.id}가 아닙니다</b> —
     ...
     * @param closedAt 캠페인이 실제로 종료된 시각. {@code null}일 수 없습니다
     */
    void retireCampaign(long campaignCouponId, Instant closedAt);
}
