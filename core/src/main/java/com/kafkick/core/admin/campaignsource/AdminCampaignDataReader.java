...
/** 관리자 운영현황과 캠페인 상세가 소비할 DB 캠페인 조회 경계입니다. */
public interface AdminCampaignDataReader {

    /**
     * 하나의 운영현황 스냅샷에 사용할 캠페인 카탈로그를 조회합니다.
     *
     * <p>이 계약은 FINAL 정합성 상태나 조치 후보를 표현하지 않습니다. A-F6 전까지 조회 구현체는
     * FINAL 후보를 합성하지 않습니다.</p>
     *
     * @param snapshotAt 조회 결과가 속할 운영현황 기준 시각
     * @return 캠페인 목록과 DB 관측 상태
     */
    AdminCampaignCatalog loadCatalog(Instant snapshotAt);

    /**
     * 한 캠페인의 상세 지표 원천을 지정한 전이 집계 구간으로 조회합니다.
     *
     * @param couponId 조회할 캠페인 식별자
     ...
     */
    AdminCampaignDetailData findDetail(
            long couponId,
