...
/**
 * 관리자 쿠폰 회차 상세 지표의 기준 시각·원천 조회·순수 계산 흐름을 조립합니다.
 *
 * <p>DB 권위값은 기술 중립 {@link AdminCouponRoundDataReader}에서 읽고, OPEN 쿠폰 회차의 발급률은
 * {@link CouponIssuanceRateReader}에서 읽고, OPEN 쿠폰 회차의 대기열은
...
public class AdminCouponMetricsService {
     ...
     * @param couponRoundDataReader Overview와 같은 DB 모집단의 상세 조회 경계
     * @param issuanceRateReader OPEN 쿠폰 회차의 Prometheus 초당 발급률 조회 경계
     * @param queueObservationSource OPEN 쿠폰 회차의 대기열 관측 조회 경계
    ...
    public AdminCouponMetricsService(
        ...
        this(timeProvider, couponRoundDataReader, issuanceRateReader, queueObservationSource, calculator,
    ...
    /**
     * 현재 시점의 한 쿠폰 회차 상세 지표를 요청 구간에 맞춰 반환합니다.
     *
     * @param couponId 조회할 쿠폰 ID
     * @param window 발급률과 상태 전이율 계산 구간
     * @return 계산 완료된 쿠폰 회차 상세 지표
     * @throws BusinessException 쿠폰 회차가 없거나 DB 관측이 불가능한 경우
     */
    public CouponMetricsSnapshot getCouponMetrics(long couponId, MetricsWindow window) {
        ...
        AdminCouponRoundDetailData detail = stockResolver.resolve(couponRoundDataReader.findDetail(
                couponId, fromInclusive, snapshotAt, snapshotAt), snapshotAt);
        if (detail.availability() == DetailAvailability.NOT_FOUND) {
            throw new BusinessException(
                    CommonErrorCode.NOT_FOUND, "상세 지표 쿠폰 회차를 찾을 수 없습니다: " + couponId);
        }
        if (detail.availability() == DetailAvailability.UNAVAILABLE) {
            throw new BusinessException(AdminCouponRoundDataErrorCode.OBSERVATION_UNAVAILABLE);
        }
        AdminCouponRoundDetailData.DetailValue value = detail.value();
        CouponMetricsSource source = new CouponMetricsSource(
                value.couponId(), value.couponRound(), value.stock(), issuanceRate(value, window, snapshotAt),
    ...
    /** OPEN 상태일 때만 외부 rate를 읽고 그 밖의 쿠폰 회차는 값 없는 N_A로 고정합니다. */
    private CouponMetricsSource.Observation<List<CouponMetricsSource.IssuanceRateSample>> issuanceRate(
            AdminCouponRoundDetailData.DetailValue value,
            MetricsWindow window,
            Instant snapshotAt
    ) {
        if (value.couponRound().status() != CouponRoundStatus.OPEN) {
            // 비 OPEN 쿠폰 회차는 관측계 질의 자체를 만들지 않아 N_A 의미를 보존합니다.
            return notApplicable();
        }
        return issuanceRateReader.read(value.couponId(), window, snapshotAt);
    }

    /** OPEN 쿠폰 회차의 요청 구간 대기열을 읽고 공통 관측을 상세 계산기 입력으로 변환합니다. */
    private CouponMetricsSource.Observation<CouponMetricsSource.QueueCounts> queue(
        ...
        if (value.couponRound().status() != CouponRoundStatus.OPEN) {
            // 비 OPEN 쿠폰 회차는 대기열 기능의 적용 대상이 아니므로 원천에 질의하지 않습니다.
            return notApplicable();
        }
        java.util.Map<Long, CouponRoundQueueObservation> observations = queueObservationSource.observe(
                List.of(value.couponId()), windowStart, snapshotAt, snapshotAt);
        CouponRoundQueueObservation observation = observations == null ? null : observations.get(value.couponId());
        if (observations == null || observations.size() != 1 || observation == null
            ...
            throw new BusinessException(
                    AdminCouponRoundDataErrorCode.OBSERVATION_UNAVAILABLE,
                    "대기열 관측 응답이 현재 상세 쿠폰 회차와 일치해야 합니다.");
        }
    ...
    /** 비 OPEN 쿠폰 회차에 값을 싣지 않는 N_A 발급률 원천을 만듭니다. */
