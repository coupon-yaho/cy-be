...
/**
 * O1 관측을 요청할 쿠폰 회차의 상태와 발급 중단 판정용 재고 관측입니다.
 *
 * <p>진행 중인 쿠폰 회차는 재고가 남았는지 또는 아직 값을 얻지 못한 상태를 손실 없이 명시합니다.
 * 아직 시작하지 않았거나 종료한 쿠폰 회차는 발급 흐름 판정 대상이 아니므로 N_A/null만 갖습니다.</p>
 *
 * @param couponId 관측할 쿠폰 회차의 양수 쿠폰 식별자
 * @param couponRoundStatus 요청 시점의 쿠폰 회차 상태
 * @param stockAvailable 진행 쿠폰 회차의 명시 재고 가능 여부; 그 밖의 상태와 값 없는 원천에서는 null
 * @param stockStatus 재고 가능 여부의 명시 원천 상태
 * @param stockObservedAt 값 있는 재고 가능 여부를 실제 관측한 시각; 값 없는 원천과 비진행 상태에서는 null
 */
public record CouponRoundObservationTarget(
        Long couponId,
    ...
    /** 쿠폰 회차 식별자와 상태별 재고 값·원천 상태의 canonical 조합만 허용합니다. */
    public CouponRoundObservationTarget {
        ...
        if (couponRoundStatus == CouponRoundStatus.OPEN) {
            if (stockStatus == SourceStatus.N_A) {
                throw new IllegalArgumentException("진행 중인 쿠폰 회차의 stockStatus는 N_A일 수 없습니다.");
            }
            ...
            throw new IllegalArgumentException("진행 중이 아닌 쿠폰 회차의 재고는 N_A/null/null이어야 합니다.");
        }
    }
    ...
    public CouponRoundObservationTarget(
        ...
        this(couponId, couponRoundStatus, stockAvailable, stockStatus, null);
    }
