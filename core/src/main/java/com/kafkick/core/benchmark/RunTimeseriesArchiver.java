...
public class RunTimeseriesArchiver {
    ...
    public enum Metric {
         ...
         *
         * <p><b>true 라고 침묵하지는 않는다.</b> 축이 비면 행을 안 쓰는 대신 이유 한 줄을
         * 남긴다({@code emptyAxisMarker}) — 계측돼 있었으면 {@link State#N_A}, 아니면
         * {@link State#UNAVAILABLE} 이다. 0행으로 두면 "그런 일이 없었다" 와 "아예 못 쟀다" 가
         * 같은 모양이 되고, archive 는 불변이라 사후 구분이 안 된다.</p>
         *
         * <p>⚠️ 대가는 {@link RangeSource#sourceExists} 질의 하나다. 그 질의에 답할 수 없으면
         * 회차 archive 가 실패한다 — 추측한 값을 적으면 그 거짓이 영구히 남기 때문이다.</p>
         */
    ...
    public interface RangeSource {
         ...
         *
         * <p>축의 표본이 비었을 때만 묻는다. 그 축이 계측돼 있었다면 빈 것은 "그런 일이
         * 없었다" 이고, 계측 자체가 없었다면 "재지 못했다" 다 — 둘은 운영자가 취할 행동이
         * 반대다.</p>
         *
         * <p><b>미터 계열이 아니라 이 축을 봐야 한다.</b> 계열만 보면 <b>축 하나만 계측되지
         * 않은 배포</b>가 "그 일이 0건이었다" 로 기록된다 — 실측으로 확인한 상태다.</p>
         *
