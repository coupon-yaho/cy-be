...
/**
 * 알림 발행 명령을 <b>언제 다시 집을지</b>. 두 경로가 이 한 벌을 공유한다 —
 * 발행 실패({@code infra:mq} 릴레이)와 lease 만료 회수({@code storage} 어댑터).
 * 왜 {@code core} 인지는 {@link NotificationRetryBackOffConfig} 에 적었다.
 */
@ConfigurationProperties("notification.outbox.retry")
public class NotificationRetryBackOffProperties {
     ...
     * 길면 정상 회복이 느려진다. 상한({@link #cap})이 폭주를 막으므로 여기는 짧게 둔다.
     */
     ...
     */
    private Duration cap = Duration.ofSeconds(20);
    ...
    public void setBase(Duration base) {
        this.base = requirePositive(base, "backoff base");
    }
    ...
    /**
     * @throws IllegalArgumentException {@code null}·0·음수이거나 365일을 넘을 때.
     *         {@link #setBase} 와 같은 시점에 터진다
     */
    public void setCap(Duration cap) {
        this.cap = requirePositive(cap, "backoff cap");
    }
