 ...
 *
 * <p>⚠️ <b>{@code .distinct()} 는 이 테스트가 못 잡는다.</b> 지우면 같은 인스턴스에
 * {@code DELETE} 가 두 번 나가는데 둘째가 0행이라 <b>합계가 그대로다</b> — 돌연변이를 넣어
 * 확인했고 초록이었다. 그 줄은 정확성이 아니라 <b>중복 문장을 안 보내는 것</b>이고,
 * 관측 가능한 차이가 없어서 잴 방법이 없다. 여기 적어 두는 것이 지금 할 수 있는 전부다.
 *
