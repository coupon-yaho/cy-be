 ...
 *
 * <p><b>그리고 이것은 설계 결정과 어긋난 현재 동작이다.</b> 설계는 <i>"데이터가 틀렸다는
 * 판정이 나와도 배치는 정상 종료다. 배치가 실패했다고 할 때는 판정을 내지 못한 경우뿐이다"</i>
 * 로 정했다. 여기서 죽는 두 가드({@code STOCK_ROW_MISSING}·{@code STOCK_UNDERFLOW})는
 * <b>데이터가 이미 어긋나 있다</b>는 판정이지 판정 불가가 아니다 — 원칙대로라면 그 회차만
 * 빼고 나머지는 진행하고 알림으로 알려야 한다.
 *
 * <p>지금은 사람이 재고를 손보는 것 말고 방법이 없다. 이 테스트는 그 대가를 기록해 둔다 —
 * 회차 단위 격리를 도입하는 날 이 단언이 뒤집히면서 도입 사실을 알린다.
 * 그 티켓의 범위는 {@code docs/13-batch-follow-ups.md} 1번에 적어 뒀다.
 */
