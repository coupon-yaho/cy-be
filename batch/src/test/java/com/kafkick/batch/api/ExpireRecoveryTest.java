...
class ExpireRecoveryTest {
    ...
    /**
     * <b>남의 잡과 없는 실행을 같은 404 로 접는다.</b> 앞단이 구성에 따라 없거나 얇아서, 남의 잡이라고
     ...
     * 두 축이 같다.
     *
     * <p><b>여기 심는 시각에 고정 시계를 쓰지 않는다.</b> 저장소 규약이 <i>"시각에 의존하는
     * 테스트는 시계를 고정한다"</i> 인데, {@link RunningJobFixture} 가 그 예외를 적어 뒀다 —
     * <i>"고정 시계는 여기서 쓸 수 없다. 조회 창이 DB 의 {@code NOW()} 를 기준으로 자르므로
     * 심는 값이 그 시계와 같은 흐름 위에 있어야 한다."</i> 게다가 <b>이 테스트의 단언은
     * 시각에 의존하지 않는다</b> — 임계를 DB 에서 읽은 {@code START_TIME} 에서 상대로
     * 만들기 때문에, 언제 돌든 대소 관계가 같다. 심는 값은 <i>"방금"</i> 이기만 하면 된다.
     */
    @Test
