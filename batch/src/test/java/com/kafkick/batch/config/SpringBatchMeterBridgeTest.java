...
class SpringBatchMeterBridgeTest {
     ...
     *
     * <p>{@code removeJobExecutions()}(인자 없는 것)는 <b>배치 메타를 전부</b> 지우는데,
    ...
    void batchMetersReachTheActuatorRegistry() throws Exception {
        started = jobOperator.start(cleanupJob, new JobParametersBuilder()
                ...
                .addString("probe", "meter-bridge-" + UUID.randomUUID())
                .addLong("attempt", 1L)
                .toJobParameters());
        assertThat(started.getStatus()).isEqualTo(BatchStatus.COMPLETED);
