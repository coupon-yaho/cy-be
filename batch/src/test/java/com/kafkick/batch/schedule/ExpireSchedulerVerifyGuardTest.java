 ...
 *
 * <p><b>건너뛰는 데는 상한이 필요 없다.</b> 스케줄러는 주기마다 {@code asOf} 를 새로 잡고
 * 만료는 {@code expires_at < asOf} 를 {@code id > 0} 부터 훑으므로, 건너뛴 슬롯의 몫을
 * 다음 슬롯이 통째로 가져간다 — 최대 지연이 정확히 크론 주기 하나다.
 * 한때 <i>"근거 있는 상한을 정할 수 없어서 반대 방향 가드를 안 단다"</i> 고 적었는데,
 * 그 논거는 <b>차단</b>을 상정한 것이라 <b>건너뛰기</b>에는 적용되지 않는다.
 *
