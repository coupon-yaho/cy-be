...
public class StuckRunSweepService {
     ...
     * <b>끝까지 돈 주기마다 부른다.</b> 안 부르면 한 번 밀린 값이 그대로 굳어, 상한을 올려
     * 해소한 뒤에도 관제가 계속 밀려 있다고 말한다.
     *
     * <p>⚠️ <b>보장은 "활성이고 끝까지 돈 주기" 까지다.</b>
     * {@code batch.stuck-sweep.enabled=false} 면 {@code StuckRunSweeper.sweep} 이 이 호출
     * <b>전에</b> 반환하므로 게이지가 마지막 값을 그대로 든다 — 그것이 맞는 답이다:
     * 아무도 안 걷는 동안 그 적체는 실제로 남아 있고, 0 으로 내리면 <b>없다고 거짓말</b>한다.
     * 왜 안 줄어드는지는 {@code cy_batch_stuck_sweep_enabled} 가 진다.
     *
