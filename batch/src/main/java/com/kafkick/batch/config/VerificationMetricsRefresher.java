 ...
 *
 * <p><b>만료 지표도 같은 모양이 됐다</b>(CY-421) — {@code ExpirePendingRefresher} 가
 * 60초마다 <i>마지막으로 성공한 실행의 {@code asOf}</i> 로 다시 센다. 한때 그쪽만
 * {@code afterJob} 에서 채웠고, 만료가 5분 크론이라 재기동 뒤 몇 분이면 값이 돌아온다는
 * 것이 근거였다. 배치 창(일 1회)으로 옮기면서 그 근거가 깨졌다.
 *
 * <p>검증은 그와 별개로 다르다. <b>사람이 손으로, 드물게</b> 돌리므로 프로세스 게이지로 두면
