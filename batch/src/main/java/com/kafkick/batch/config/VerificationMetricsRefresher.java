 ...
 *
 * <p>만료 지표는 {@code afterJob} 에서 채운다 — 만료가 <b>5분 크론이었기 때문</b>이다.
 *
 * <p>⚠️ <b>그 근거는 CY-397 이 만료를 배치 창(일 1회)으로 옮기면서 깨졌다.</b>
 * {@code cy_expire_*} 는 이제 재기동부터 다음 04:10 까지 <b>최대 하루</b> 동안 {@code NaN}
 * 이고, 그 사이 {@code ExpireLeavesWorkBehind} 가 발화할 수 없다 — 이 클래스가 검증 지표를
 * 되읽기로 만든 이유와 <b>정확히 같은 문제</b>다. 만료 지표도 같은 모양으로 옮겨야 한다
 * ({@code docs/13} §6 에 항목으로 남겼다). 검증은 그와 별개로 다르다. <b>사람이 손으로, 드물게</b> 돌리므로 프로세스 게이지로 두면
