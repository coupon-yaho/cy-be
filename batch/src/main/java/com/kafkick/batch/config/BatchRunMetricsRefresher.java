...
public class BatchRunMetricsRefresher {
     ...
     * 보인다</b> — 창이 그 자리를 겸한다.
     *
     * <p><b>{@code YIELDED} 는 성공이 아니다.</b> 정리 잡은 검증이 도는 중이면 <b>검증 파생
     * 행을 한 행도 안 걷고</b> {@code COMPLETED} 로 닫힌다(배치 메타는 그래도 걷는다 —
     * {@code purgeBatchMetadataStep} 은 검증 데이터를 안 건드린다). 실패가 아니므로 상태를
     * 바꿀 수 없지만, 남은 백로그가 곧 다음 날의 일이라
