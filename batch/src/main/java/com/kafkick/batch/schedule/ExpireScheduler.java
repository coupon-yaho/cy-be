...
public class ExpireScheduler {
     ...
     * 지금 이 잡뿐이고, 그 전제가 깨지면 락 순서 계약({@code issuances} →
     * {@code issuance_histories} → {@code coupon_stocks})을 그쪽도 지켜야 한다 —
     * {@code ExpirationRepository} 에 적어 뒀다. <b>그 순서는 우리가 고른 것이 아니다</b> —
     * 발급·취소가 재고를 마지막에 건드리고, 만료만 반대였을 때 취소가 1213 으로
     * 죽는 것을 재현했다({@code docs/12-expire-lock-measurement.md} §11·§11.1).
     *
