...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
     ...
     *
     * <p>같은 400 이라도 보통의 {@code BusinessException} 은 카탈로그 문구만 나가고 detail 은
     * 로그에 남는다 — detail 에 식별자가 들어 있을 수 있어서다. 헤더 계약은 그 반대다.
     * <b>호출자가 무엇을 고쳐야 하는지 응답만 보고 알아야</b> 하고, 사유는 열거형이 정한
     * 고정 문구라 요청 내용을 되비추지 않는다. 제약 애노테이션 문구를 싣는 것과 같은 등급이다.
     *
     * <p><b>이 처리기가 아래 {@code BusinessException} 처리기보다 먼저 걸린다</b> — 스프링이
     * 더 좁은 타입을 고르기 때문이다. 상속 관계를 둔 것은 발급 컨트롤러가 이것을 장애가 아니라
     * 입력 거절로 집계하게 하려는 것이고, 응답 모양은 이 처리기가 정한다.
     *
