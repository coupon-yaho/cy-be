...
/**
 * 쿠폰 회차 수명 통지를 버리는 기본 구현입니다.
 *
 * <p>값 계약 위반도 호출자에게 던지지 않고 WARN 으로만 남깁니다. 종료 통지는 쿠폰 회차 종료
 ...
 */
public final class NoOpCouponRoundLifecycleRecorder implements CouponRoundLifecycleRecorder {

    private static final Logger log =
            LoggerFactory.getLogger(NoOpCouponRoundLifecycleRecorder.class);

    @Override
    public void retireCouponRound(long couponId, Instant closedAt) {
        if (couponId <= 0 || closedAt == null) {
            log.warn(
                    "쿠폰 회차 수명 통지의 값 계약을 위반했습니다. 통지를 버리고 계속 진행합니다. "
                            + "couponId={}, closedAt={}",
                    couponId,
                    closedAt
            );
