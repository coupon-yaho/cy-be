...
public class PromQueryClient implements PromQuery, PromTimeQuery, RunTimeseriesArchiver.RangeSource {
    ...
    /**
     * 축이 빈 이유를 가르는 <b>원천 존재 질의</b>입니다.
     *
     * <p><b>축 라벨을 빼고 미터 계열만 봅니다.</b> {@code outcome} 을 걸면 "그 축이 비었다" 를
     * 다시 확인할 뿐이라 아무것도 못 가릅니다 — 물어야 하는 것은 <b>미터 자체가 그 구간에
     * 있었는가</b> 입니다.</p>
     *
     * <p>{@code count_over_time} 을 씁니다. 표본이 하나라도 있었으면 결과 벡터가 나오고, 미터가
     * 아예 없었으면 빈 벡터입니다 — 값이 몇인지는 안 봅니다.</p>
     *
