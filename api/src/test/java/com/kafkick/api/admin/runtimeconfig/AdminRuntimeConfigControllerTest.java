...
import com.kafkick.core.runtimeconfig.InMemoryRuntimeConfigStore;
...
class AdminRuntimeConfigControllerTest {
    ...
    void exposesGetAndFullPutContracts() throws Exception {
        MockMvc mockMvc = mockMvc(inMemoryStore());
        ...
        mockMvc.perform(put("/api/v1/admin/runtime-config")
                ...
                .andExpect(jsonPath("$.error.code").value("RUNTIME_CONFIG-003"))
                .andExpect(jsonPath("$.error.currentRevision").value(3));
    }

    /** 현재 revision을 전달한 전체 교체가 새 revision과 Caller 변경 주체를 반환하는지 검증합니다. */
    @Test
    void updateReturnsCasSnapshotWithIncrementedRevisionAndCallerMemberId() throws Exception {
        MockMvc mockMvc = mockMvc(inMemoryStore());

        mockMvc.perform(put("/api/v1/admin/runtime-config")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"expectedRevision\":3,\"engineVersion\":\"V2\","
                                + "\"releaseStage\":\"V2_2\",\"queueMode\":\"ALWAYS\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.revision").value(4))
                .andExpect(jsonPath("$.data.engineVersion").value("V2"))
                .andExpect(jsonPath("$.data.releaseStage").value("V2_2"))
                .andExpect(jsonPath("$.data.queueMode").value("ALWAYS"))
                .andExpect(jsonPath("$.data.updatedAt").value("2026-08-26T01:00:00Z"))
                .andExpect(jsonPath("$.data.updatedBy").value("812934"))
                .andExpect(jsonPath("$.data.sourceStatus").value("VALID"));
    }

    /** 읽기 전용 Store의 PUT이 다른 revision 충돌과 구분되는 오류 코드를 유지하는지 검증합니다. */
    @Test
    void updateRetainsReadOnlyConflictCode() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/admin/runtime-config")
                        ...
                        .content("{\"expectedRevision\":3,\"engineVersion\":\"V2\","
                                + "\"releaseStage\":\"V2_2\",\"queueMode\":\"ALWAYS\"}"))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.error.code").value("RUNTIME_CONFIG-005"));
    }

    /** 조회 Store 장애가 Runtime Config 저장소 오류 코드와 503 상태를 보존하는지 검증합니다. */
    @Test
    void getRetainsStoreUnavailableServiceResponse() throws Exception {
        MockMvc mockMvc = mockMvc(unavailableStore());

        mockMvc.perform(get("/api/v1/admin/runtime-config"))
                .andExpect(status().isServiceUnavailable())
                .andExpect(jsonPath("$.error.code").value("RUNTIME_CONFIG-004"));
    }
    ...
    void fullPutRejectsMissingRevisionOrAnyConfigurationField() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/admin/runtime-config")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"expectedRevision\":1,\"engineVersion\":\"V3\",\"queueMode\":\"ADAPTIVE\"}"))
                .andExpect(status().isBadRequest());
        mockMvc.perform(put("/api/v1/admin/runtime-config")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"expectedRevision\":1,\"engineVersion\":\"V3\",\"releaseStage\":\"V3\"}"))
                .andExpect(status().isBadRequest());
        mockMvc.perform(put("/api/v1/admin/runtime-config")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"expectedRevision\":-1,\"engineVersion\":\"V3\","
    ...
    private static MockMvc mockMvc(RuntimeConfigSnapshot snapshot) {
        return mockMvc(new ReadOnlyRuntimeConfigStore(snapshot));
    }

    private static MockMvc mockMvc(RuntimeConfigStore runtimeConfigStore) {
        return AdminControllerContractTestSupport.mockMvc(new AdminRuntimeConfigController(runtimeConfigStore));
    }

    private static RuntimeConfigStore inMemoryStore() {
        return new InMemoryRuntimeConfigStore(
                snapshot(SourceStatus.VALID),
                Clock.fixed(Instant.parse("2026-08-26T01:00:00Z"), ZoneOffset.UTC));
    }

    private static RuntimeConfigStore unavailableStore() {
        return new RuntimeConfigStore() {
            @Override
            public RuntimeConfigSnapshot get() {
                throw new BusinessException(RuntimeConfigErrorCode.STORE_UNAVAILABLE);
            }
