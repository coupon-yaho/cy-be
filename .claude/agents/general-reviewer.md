...
**[major/high] 이벤트 스트림에 회원 이름이 노출된다**
무엇이     event.memberName 을 그대로 렌더링한다
근거       `dashboard.js:112`
왜 문제    개인정보 마스킹은 과제가 명시적으로 요구하는 유일한 보안 요건이다
언제 터지나 화면을 켜는 순간 노출되고, 시연 영상에도 그대로 녹화된다
어떻게     `memberId` 만 표시한다. 내부 식별자라 마스킹 대상이 아니다

**[minor/medium] k6 VU 수가 스크립트에 하드코딩돼 있다**
무엇이     VU 20000 이 상수로 박혀 있다
근거       `s1-burst.js:8`
왜 문제    L1·L2·L3 환경마다 부하 규모가 달라야 하는데 스크립트를 고쳐야 한다
언제 터지나 환경을 바꿔 측정할 때 스크립트가 갈라지고 비교표의 조건 고정이 깨진다
어떻게     `__ENV.VUS` 로 받고 기본값만 스크립트에 둔다
